...
import {
  View, Text, Picker, StyleSheet, ScrollView, Button, Item, FlatList
} from 'react-native';
      ...
      <View>
        <View>
          <Text style={styles.text}>
            Choose from as many courses as you're intersted in.
          </Text>
        </View>
        <View style={{
          height: 150, marginTop: 30, backgroundColor: '#909090', width: 200, alignSelf: 'center', borderRadius: 10
        }}
        >
          <FlatList
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View key={item.index} style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
                ...
                <Button onPress={() => { this.handleRemove(item.key); }} title='x' />
              </View>
            )}
          />
        </View>
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          <View style={{
            height: 40, width: 100, backgroundColor: nextButtonColor, borderRadius: 5, marginTop: 20
          }}
          >
            <Button
              title='Submit'
